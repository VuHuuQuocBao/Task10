...
namespace Task10_11.Controllers
{
    ...
    public class NewController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll()
        {
            try
            {
                return Ok(await _NewService.GetAll());
            }
            catch (Exception ex)
            {
        ...
        {
            var products = await _NewService.GetAllPaging(pagingRequest);
            return Ok(products);
        }

        [HttpPost]
        public async Task<IActionResult> Create(NewDTO newDTO)
        {
            try
            {
                return Ok(await _NewService.Add(newDTO));
            }
            catch (Exception ex)
            {
                return BadRequest(ex.Message);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(NewDTOUpdateRequest newDTO)
        {
            try
            {
                var result = await _NewService.Update(newDTO);
                if (result == 0)
                    return BadRequest();
